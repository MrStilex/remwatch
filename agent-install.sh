...
if [ -n "$CONFLICT" ]; then
  echo ""
  warn "Обнаружен уже установленный Vector (${CONFLICT})."
  warn "Удаляю, чтобы избежать конфликтов и дублирования логов..."
...
info "Запускаю Vector..."
docker compose up -d

# ─── проверка доставки логов ───────────────────────────────────────────────────
echo ""
info "Жду запуска Vector (10 сек)..."
sleep 10
